...
module.exports = function(options, env = 'development') {
  const hasPostcssConfig = fs.existsSync(options.paths.appPostcss)

  return merge(createBaseConfig(options, 'web', env), {
    entry: {
      app: [options.paths.ownPolyfillsPath, options.paths.clientEntry],
    },
    ...
    plugins: [
      new AssetsPlugin({
        output: options.paths.appPublicManifest,
        publicPath: true,
        customize: entry => ({
